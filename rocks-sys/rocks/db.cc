...
uint32_t rocks_column_family_handle_get_id(const rocks_column_family_handle_t* handle) { return handle->rep->GetID(); }
}
...
rocks_db_t* rocks_db_open_column_families(const rocks_options_t* db_options, const char* name, int num_column_families,
  ...
  if (SaveError(status, DB::Open(DBOptions(db_options->rep), std::string(name), column_families, &handles, &db))) {
    return nullptr;
  }
...
rocks_db_t* rocks_db_open_for_read_only_column_families(const rocks_options_t* db_options, const char* name,
  ...
  DB* db;
  std::vector<ColumnFamilyHandle*> handles;
  if (SaveError(status, DB::OpenForReadOnly(DBOptions(db_options->rep), std::string(name), column_families, &handles,
...
char** rocks_db_list_column_families(const rocks_options_t* options, const char* name, size_t* lencfs,
  ...
  auto st = DB::ListColumnFamilies(DBOptions(options->rep), std::string(name), &fams);
  if (SaveError(status, std::move(st))) {
    *lencfs = 0;
    return nullptr;
  }
...
void rocks_db_put(rocks_db_t* db, const rocks_writeoptions_t* options, const char* key, size_t keylen, const char* val,
  ...
  SaveError(status, db->rep->Put(options->rep, Slice(key, keylen), Slice(val, vallen)));
}
...
void rocks_db_put_cf(rocks_db_t* db, const rocks_writeoptions_t* options, rocks_column_family_handle_t* column_family,
  ...
  SaveError(status, db->rep->Put(options->rep, column_family->rep, Slice(key, keylen), Slice(val, vallen)));
}
...
void rocks_db_delete_cf(rocks_db_t* db, const rocks_writeoptions_t* options,
                        ...
                        rocks_status_t** status) {
  SaveError(status, db->rep->Delete(options->rep, column_family->rep, Slice(key, keylen)));
}
...
void rocks_db_single_delete_cf(rocks_db_t* db, const rocks_writeoptions_t* options,
                               rocks_column_family_handle_t* column_family, const char* key, size_t keylen,
                               rocks_status_t** status) {
  SaveError(status, db->rep->SingleDelete(options->rep, column_family->rep, Slice(key, keylen)));
}
...
void rocks_db_delete_range_cf(rocks_db_t* db, const rocks_writeoptions_t* options,
  ...
  SaveError(status, db->rep->DeleteRange(options->rep, column_family->rep, Slice(begin_key, begin_keylen),
...
void rocks_db_merge(rocks_db_t* db, const rocks_writeoptions_t* options, const char* key, size_t keylen,
  ...
  SaveError(status, db->rep->Merge(options->rep, Slice(key, keylen), Slice(val, vallen)));
}

void rocks_db_merge_cf(rocks_db_t* db, const rocks_writeoptions_t* options, rocks_column_family_handle_t* column_family,
                       const char* key, size_t keylen, const char* val, size_t vallen, rocks_status_t** status) {
  SaveError(status, db->rep->Merge(options->rep, column_family->rep, Slice(key, keylen), Slice(val, vallen)));
}
...
void rocks_db_get_pinnable(rocks_db_t* db, const rocks_readoptions_t* options, const char* key, size_t keylen,
  ...
  Status st = db->rep->Get(options->rep, db->rep->DefaultColumnFamily(), Slice(key, keylen), &value->rep);
  SaveError(status, std::move(st));
}
...
void rocks_db_get_cf_pinnable(rocks_db_t* db, const rocks_readoptions_t* options,
                              rocks_column_family_handle_t* column_family, const char* key, size_t keylen,
                              rocks_pinnable_slice_t* value, rocks_status_t** status) {
  Status st = db->rep->Get(options->rep, column_family->rep, Slice(key, keylen), &value->rep);
  SaveError(status, std::move(st));
}
...
unsigned char rocks_db_key_may_exist(rocks_db_t* db, const rocks_readoptions_t* options, const char* key,
  ...
  bool found;
  std::string val;
  bool has_value = db->rep->KeyMayExist(options->rep, Slice(key, key_len), &val, &found);
...
unsigned char rocks_db_key_may_exist_cf(rocks_db_t* db, const rocks_readoptions_t* options,
  ...
  if (value_found != nullptr) {
    std::string val;
    bool has_value =
        db->rep->KeyMayExist(options->rep, column_family->rep, Slice(key, key_len), &val, (bool*)value_found);
...
void rocks_db_compact_range(rocks_db_t* db, const char* start_key, size_t start_key_len, const char* limit_key,
  ...
  Slice a, b;
  db->rep->CompactRange(CompactRangeOptions(),
...
void rocks_db_compact_range_cf(rocks_db_t* db, rocks_column_family_handle_t* column_family, const char* start_key,
  ...
  Slice a, b;
  db->rep->CompactRange(CompactRangeOptions(), column_family->rep,
                        // Pass nullptr Slice if corresponding "const char*" is nullptr
                        (start_key ? (a = Slice(start_key, start_key_len), &a) : nullptr),
                        (limit_key ? (b = Slice(limit_key, limit_key_len), &b) : nullptr));
}
...
void rocks_db_compact_range_opt(rocks_db_t* db, rocks_compactrange_options_t* opt, const char* start_key,
                                ...
                                rocks_status_t** status) {
  Slice a, b;
  auto st = db->rep->CompactRange(opt->rep,
                                  // Pass nullptr Slice if corresponding "const char*" is nullptr
                                  (start_key ? (a = Slice(start_key, start_key_len), &a) : nullptr),
                                  (limit_key ? (b = Slice(limit_key, limit_key_len), &b) : nullptr));
  SaveError(status, std::move(st));
...
void rocks_db_compact_range_opt_cf(rocks_db_t* db, rocks_compactrange_options_t* opt,
                                   ...
                                   rocks_status_t** status) {
  Slice a, b;
  auto st = db->rep->CompactRange(opt->rep, column_family->rep,
                                  // Pass nullptr Slice if corresponding "const char*" is nullptr
                                  (start_key ? (a = Slice(start_key, start_key_len), &a) : nullptr),
                                  (limit_key ? (b = Slice(limit_key, limit_key_len), &b) : nullptr));
  SaveError(status, std::move(st));
...
void rocks_db_set_options_cf(rocks_db_t* db, rocks_column_family_handle_t* column_family, size_t num_options,
  ...
  for (auto i = 0; i < num_options; i++) {
    new_options[std::string(keys[i], key_lens[i])] = std::string(vals[i], val_lens[i]);
  }
...
int rocks_db_max_mem_compaction_level(rocks_db_t* db) { return db->rep->MaxMemCompactionLevel(); }
...
int rocks_db_level0_stop_write_trigger(rocks_db_t* db) { return db->rep->Level0StopWriteTrigger(); }
...
void rocks_db_get_approximate_sizes_cf(rocks_db_t* db, rocks_column_family_handle_t* column_family, size_t num_ranges,
  ...
  for (int i = 0; i < num_ranges; i++) {
    ranges.push_back(
        Range(Slice(range_start_ptrs[i], range_start_lens[i]), Slice(range_limit_ptrs[i], range_limit_lens[i])));
  }
  db->rep->GetApproximateSizes(column_family->rep, ranges.data(), num_ranges, sizes);
}
...
void rocks_db_get_approximate_memtable_stats_cf(rocks_db_t* db, rocks_column_family_handle_t* column_family,
                                                ...
                                                uint64_t* size) {
  auto range = Range(Slice(range_start_ptr, range_start_len), Slice(range_limit_ptr, range_limit_len));
  db->rep->GetApproximateMemTableStats(column_family->rep, range, count, size);
}

void rocks_db_compact_range_cf_opt(rocks_db_t* db, rocks_column_family_handle_t* column_family,
                                   rocks_compactrange_options_t* opt, const char* start_key, size_t start_key_len,
                                   const char* limit_key, size_t limit_key_len, rocks_status_t** status) {
  Slice a, b;
  auto st = db->rep->CompactRange(opt->rep, column_family->rep,
                                  // Pass nullptr Slice if corresponding "const char*" is nullptr
                                  (start_key ? (a = Slice(start_key, start_key_len), &a) : nullptr),
...
void rocks_db_flush_cf(rocks_db_t* db, rocks_flushoptions_t* options, rocks_column_family_handle_t* column_family,
                       rocks_status_t** status) {
  SaveError(status, std::move(db->rep->Flush(options->rep, column_family->rep)));
}

void rocks_db_sync_wal(rocks_db_t* db, rocks_status_t** status) { SaveError(status, std::move(db->rep->SyncWAL())); }

uint64_t rocks_db_get_latest_sequence_number(rocks_db_t* db) { return db->rep->GetLatestSequenceNumber(); }
...
cxx_string_vector_t* rocks_db_get_live_files(rocks_db_t* db, unsigned char flush_memtable, uint64_t* manifest_file_size,
  ...
  auto st = db->rep->GetLiveFiles(files->rep, manifest_file_size, flush_memtable);
  if (SaveError(status, std::move(st))) {
    delete files;
    return nullptr;
  }
...
rocks_table_props_collection_t* rocks_db_get_properties_of_tables_in_range(
    ...
    const size_t* start_key_lens, const char* const* limit_keys, const size_t* limit_key_lens,
    rocks_status_t** status) {
  auto coll = new rocks_table_props_collection_t;
  auto ranges = std::vector<Range>{};
  for (auto i = 0; i < num_ranges; i++) {
    ranges.push_back(Range(Slice(start_keys[i], start_key_lens[i]), Slice(limit_keys[i], limit_key_lens[i])));
  }
  auto st = db->rep->GetPropertiesOfTablesInRange(cf->rep, ranges.data(), ranges.size(), &coll->rep);
  if (!SaveError(status, std::move(st))) {
    return coll;
  }
  delete coll;
