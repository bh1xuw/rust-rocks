...
rocks_options_t* rocks_options_create() { return new rocks_options_t; }

void rocks_options_destroy(rocks_options_t* options) { delete options; }

rocks_dboptions_t* rocks_dboptions_create() { return new rocks_dboptions_t; }

void rocks_dboptions_destroy(rocks_dboptions_t* options) { delete options; }
...
void rocks_cfoptions_optimize_for_small_db(rocks_cfoptions_t* opt) { opt->rep.OptimizeForSmallDb(); }
...
void rocks_cfoptions_set_merge_operator_by_assoc_op_trait(rocks_cfoptions_t* opt, void* op_trait_obj) {
  opt->rep.merge_operator = std::shared_ptr<MergeOperator>(new rocks_associative_mergeoperator_t{op_trait_obj});
}

void rocks_cfoptions_set_merge_operator_by_merge_op_trait(rocks_cfoptions_t* opt, void* op_trait_obj) {
  opt->rep.merge_operator = std::shared_ptr<MergeOperator>(new rocks_mergeoperator_t{op_trait_obj});
}
...
void rocks_cfoptions_set_compaction_filter_factory(
                                                 rocks_options_t* opt,
rocks_compactionfilterfactory_t* factory) { opt->rep.compaction_filter_factory =
    std::shared_ptr<CompactionFilterFactory>(factory);
}
*/

void rocks_cfoptions_set_write_buffer_size(rocks_cfoptions_t* opt, size_t s) { opt->rep.write_buffer_size = s; }
...
void rocks_cfoptions_set_block_based_table_factory(rocks_cfoptions_t* opt,
  ...
  if (table_options) {
    opt->rep.table_factory.reset(rocksdb::NewBlockBasedTableFactory(table_options->rep));
  }
}

void rocks_cfoptions_set_cuckoo_table_factory(rocks_cfoptions_t* opt, rocks_cuckoo_table_options_t* table_options) {
  if (table_options) {
    opt->rep.table_factory.reset(rocksdb::NewCuckooTableFactory(table_options->rep));
  }
}

void rocks_cfoptions_set_plain_table_factory(rocks_cfoptions_t* opt, rocks_plain_table_options_t* table_options) {
  if (table_options) {
    opt->rep.table_factory.reset(rocksdb::NewPlainTableFactory(table_options->rep));
  }
}

/*
void rocks_cfoptions_set_plain_table_factory(
                                           rocks_cfoptions_t *opt, uint32_t
user_key_len, int bloom_bits_per_key, double hash_table_ratio, size_t
index_sparseness) { rocksdb::PlainTableOptions options; options.user_key_len =
user_key_len; options.bloom_bits_per_key = bloom_bits_per_key;
...
void rocks_cfoptions_set_bloom_locality(rocks_cfoptions_t* opt, uint32_t v) { opt->rep.bloom_locality = v; }

void rocks_cfoptions_set_arena_block_size(rocks_cfoptions_t* opt, size_t v) { opt->rep.arena_block_size = v; }
...
void rocks_cfoptions_set_compression_per_level(rocks_cfoptions_t* opt, const int* level_values, size_t num_levels) {
  ...
  for (size_t i = 0; i < num_levels; ++i) {
    opt->rep.compression_per_level[i] = static_cast<CompressionType>(level_values[i]);
  }
}

void rocks_cfoptions_set_num_levels(rocks_cfoptions_t* opt, int n) { opt->rep.num_levels = n; }
...
void rocks_cfoptions_set_max_compaction_bytes(rocks_cfoptions_t* opt, uint64_t n) { opt->rep.max_compaction_bytes = n; }
...
void rocks_cfoptions_set_hash_skip_list_rep(rocks_cfoptions_t* opt, size_t bucket_count, int32_t skiplist_height,
  ...
  rocksdb::MemTableRepFactory* factory =
      rocksdb::NewHashSkipListRepFactory(bucket_count, skiplist_height, skiplist_branching_factor);
...
void rocks_cfoptions_add_table_properties_collector_factories_by_trait(rocks_cfoptions_t* opt,
  ...
  opt->rep.table_properties_collector_factories.push_back(
      std::shared_ptr<TablePropertiesCollectorFactory>(new rocks_table_props_collector_factory_t(factory_trait_obj)));
}

void rocks_cfoptions_set_max_successive_merges(rocks_cfoptions_t* opt, size_t v) { opt->rep.max_successive_merges = v; }
...
void rocks_dboptions_optimize_for_small_db(rocks_dboptions_t* opt) { opt->rep.OptimizeForSmallDb(); }
...
void rocks_dboptions_set_create_if_missing(rocks_dboptions_t* opt, unsigned char v) { opt->rep.create_if_missing = v; }
...
void rocks_dboptions_set_error_if_exists(rocks_dboptions_t* opt, unsigned char v) { opt->rep.error_if_exists = v; }

void rocks_dboptions_set_paranoid_checks(rocks_dboptions_t* opt, unsigned char v) { opt->rep.paranoid_checks = v; }

void rocks_dboptions_set_env(rocks_dboptions_t* opt, rocks_env_t* env) { opt->rep.env = (env ? env->rep : nullptr); }
...
rocks_sst_file_manager_t* manager) { opt->rep.sst_file_manager = manager->rep;
}
*/
...
void rocks_dboptions_set_max_open_files(rocks_dboptions_t* opt, int n) { opt->rep.max_open_files = n; }

void rocks_dboptions_set_max_file_opening_threads(rocks_dboptions_t* opt, int n) {
  opt->rep.max_file_opening_threads = n;
}

void rocks_dboptions_set_max_total_wal_size(rocks_dboptions_t* opt, uint64_t n) { opt->rep.max_total_wal_size = n; }
...
void rocks_dboptions_set_use_fsync(rocks_dboptions_t* opt, unsigned char use_fsync) { opt->rep.use_fsync = use_fsync; }
...
void rocks_dboptions_set_log_file_time_to_roll(rocks_dboptions_t* opt, size_t v) { opt->rep.log_file_time_to_roll = v; }

void rocks_dboptions_set_keep_log_file_num(rocks_dboptions_t* opt, size_t v) { opt->rep.keep_log_file_num = v; }

void rocks_dboptions_set_recycle_log_file_num(rocks_dboptions_t* opt, size_t v) { opt->rep.recycle_log_file_num = v; }
...
void rocks_dboptions_set_wal_ttl_seconds(rocks_dboptions_t* opt, uint64_t ttl) { opt->rep.WAL_ttl_seconds = ttl; }
...
void rocks_dboptions_set_allow_mmap_reads(rocks_dboptions_t* opt, unsigned char v) { opt->rep.allow_mmap_reads = v; }

void rocks_dboptions_set_allow_mmap_writes(rocks_dboptions_t* opt, unsigned char v) { opt->rep.allow_mmap_writes = v; }

void rocks_dboptions_set_use_direct_reads(rocks_dboptions_t* opt, unsigned char v) { opt->rep.use_direct_reads = v; }
...
void rocks_dboptions_set_allow_fallocate(rocks_dboptions_t* opt, unsigned char v) { opt->rep.allow_fallocate = v; }
...
void rocks_dboptions_set_db_write_buffer_size(rocks_dboptions_t* opt, size_t s) { opt->rep.db_write_buffer_size = s; }
...
void rocks_dboptions_set_bytes_per_sync(rocks_dboptions_t* opt, uint64_t v) { opt->rep.bytes_per_sync = v; }

void rocks_dboptions_set_wal_bytes_per_sync(rocks_dboptions_t* opt, uint64_t v) { opt->rep.wal_bytes_per_sync = v; }

void rocks_dboptions_add_listener(rocks_dboptions_t* opt, void* listener_trait_obj) {
  opt->rep.listeners.push_back(std::shared_ptr<EventListener>(new rocks_event_listener_t{listener_trait_obj}));
}
...
void rocks_dboptions_set_delayed_write_rate(rocks_dboptions_t* opt, uint64_t v) { opt->rep.delayed_write_rate = v; }
...
void rocks_dboptions_set_allow_2pc(rocks_dboptions_t* opt, unsigned char v) { opt->rep.allow_2pc = v; }

// FIXME: mem leaks?
void rocks_dboptions_set_row_cache(rocks_dboptions_t* opt, rocks_cache_t* cache) { opt->rep.row_cache = cache->rep; }

/*
void rocks_dboptions_set_wal_filter(rocks_dboptions_t* opt, rocks_wal_filter_t*
filter) { opt->rep.wal_filter = filter->rep;
}
*/
...
void rocks_dboptions_set_dump_malloc_stats(rocks_dboptions_t* opt, unsigned char v) { opt->rep.dump_malloc_stats = v; }
...
void rocks_options_optimize_for_small_db(rocks_options_t* opt) { opt->rep.OptimizeForSmallDb(); }
...
// rocks_readoptions_t
extern "C" {
rocks_readoptions_t* rocks_readoptions_create() { return new rocks_readoptions_t; }
...
void rocks_readoptions_destroy(rocks_readoptions_t* opt) { delete opt; }

void rocks_readoptions_set_verify_checksums(rocks_readoptions_t* opt, unsigned char v) {
  opt->rep.verify_checksums = v;
}

void rocks_readoptions_set_fill_cache(rocks_readoptions_t* opt, unsigned char v) { opt->rep.fill_cache = v; }
...
void rocks_readoptions_set_tailing(rocks_readoptions_t* opt, unsigned char v) { opt->rep.tailing = v; }

void rocks_readoptions_set_managed(rocks_readoptions_t* opt, unsigned char v) { opt->rep.managed = v; }
...
extern "C" {
rocks_writeoptions_t* rocks_writeoptions_create() { return new rocks_writeoptions_t; }

void rocks_writeoptions_destroy(rocks_writeoptions_t* opt) { delete opt; }

void rocks_writeoptions_set_sync(rocks_writeoptions_t* opt, unsigned char v) { opt->rep.sync = v; }

void rocks_writeoptions_set_disable_wal(rocks_writeoptions_t* opt, unsigned char v) { opt->rep.disableWAL = v; }
...
extern "C" {
rocks_compactrange_options_t* rocks_compactrange_options_create() { return new rocks_compactrange_options_t; }

void rocks_compactrange_options_destroy(rocks_compactrange_options_t* opt) { delete opt; }
...
void rocks_ingestexternalfile_options_destroy(rocks_ingestexternalfile_options_t* opt) { delete opt; }
...
extern "C" {
rocks_flushoptions_t* rocks_flushoptions_create() { return new rocks_flushoptions_t; }
...
rocks_logger_t* rocks_create_logger_from_options(const char* path, rocks_options_t* opts, rocks_status_t** status) {
  ...
  Status st = CreateLoggerFromOptions(std::string(path), opts->rep, &logger->rep);
...
extern "C" {
rocks_compaction_options_t* rocks_compaction_options_create() { return new rocks_compaction_options_t; }

void rocks_compaction_options_destroy(rocks_compaction_options_t* opts) { delete opts; }
