...
void rocks_cfoptions_set_merge_operator_by_merge_op_trait(
  ...
  opt->rep.merge_operator =
      std::shared_ptr<MergeOperator>(new rocks_mergeoperator_t{op_trait_obj});
}
...
void rocks_cfoptions_set_max_write_buffer_number(rocks_cfoptions_t* opt,
                                                 int n) {
...
void rocks_cfoptions_set_memtable_insert_with_hint_prefix_extractor_by_trait(
  ...
  opt->rep.memtable_insert_with_hint_prefix_extractor.reset(
      new rocks_slice_transform_t{trans_trait_obj});
}
...
void rocks_dboptions_set_max_total_wal_size(rocks_dboptions_t* opt,
                                            uint64_t n) {
...
void rocks_dboptions_set_db_paths(rocks_dboptions_t* opt,
  ...
  for (int i = 0; i < size; i++) {
    dbpaths.push_back(
        DbPath(std::string(paths[i], path_lens[i]), target_sizes[i]));
  }
...
void rocks_dboptions_set_max_subcompactions(rocks_dboptions_t* opt,
                                            uint32_t n) {
...
void rocks_dboptions_set_recycle_log_file_num(rocks_dboptions_t* opt,
                                              size_t v) {
...
void rocks_dboptions_set_wal_bytes_per_sync(rocks_dboptions_t* opt,
                                            uint64_t v) {
...
void rocks_dboptions_set_delayed_write_rate(rocks_dboptions_t* opt,
                                            uint64_t v) {
