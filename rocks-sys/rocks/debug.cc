...
rocks_key_version_collection_t* rocks_db_get_all_key_versions(
  ...
  auto st = GetAllKeyVersions(db->rep, Slice(begin_key, begin_keylen),
                              Slice(end_key, end_keylen), &coll->rep);
...
void rocks_key_version_collection_destroy(
    rocks_key_version_collection_t* coll) {
  delete coll;
}
