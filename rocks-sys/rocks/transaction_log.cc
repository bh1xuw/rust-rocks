...
size_t rocks_logfiles_size(rocks_logfiles_t* files) { return files->rep.size(); }
...
uint64_t rocks_logfiles_nth_log_number(rocks_logfiles_t* files, size_t nth) { return files->rep[nth]->LogNumber(); }

int rocks_logfiles_nth_type(rocks_logfiles_t* files, size_t nth) { return static_cast<int>(files->rep[nth]->Type()); }

uint64_t rocks_logfiles_nth_start_sequence(rocks_logfiles_t* files, size_t nth) {
  return files->rep[nth]->StartSequence();
}

uint64_t rocks_logfiles_nth_file_size(rocks_logfiles_t* files, size_t nth) { return files->rep[nth]->SizeFileBytes(); }

// rocks_transaction_log_iterator_t
void rocks_transaction_log_iterator_destory(rocks_transaction_log_iterator_t* it) { delete it; }

unsigned char rocks_transaction_log_iterator_valid(rocks_transaction_log_iterator_t* it) { return it->rep->Valid(); }

void rocks_transaction_log_iterator_next(rocks_transaction_log_iterator_t* it) { it->rep->Next(); }
