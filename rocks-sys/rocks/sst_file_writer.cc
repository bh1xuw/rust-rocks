...
rocks_external_sst_file_info_t* rocks_external_sst_file_info_create() {
  return new rocks_external_sst_file_info_t;
}

void rocks_external_sst_file_info_destroy(
...
rocks_sst_file_writer_t* rocks_sst_file_writer_create_from_c_comparator(
    ...
    const Comparator* comparator, rocks_column_family_handle_t* column_family,
  ...
  result->rep = new SstFileWriter(
      env_options->rep, options->rep, comparator,
...
rocks_sst_file_writer_t* rocks_sst_file_writer_create_from_rust_comparator(
  ...
  rocks_sst_file_writer_t* result = new rocks_sst_file_writer_t;
  result->rep = new SstFileWriter(
      env_options->rep, options->rep,
      new rocks_comparator_t{
          (void*)comparator_trait_obj},  // FIXME: memory leaks
      (column_family != nullptr) ? column_family->rep : nullptr,
      invalidate_page_cache != 0);
  return result;
}
