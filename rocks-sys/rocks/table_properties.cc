...
void rocks_table_props_collection_destroy(rocks_table_props_collection_t* coll) { delete coll; }

void rocks_table_props_destroy(rocks_table_props_t* props) { delete props; }

void rocks_table_props_collection_iter_destroy(rocks_table_props_collection_iter_t* it) { delete it; }

void rocks_user_collected_props_iter_destroy(rocks_user_collected_props_iter_t* it) { delete it; }

size_t rocks_table_props_collection_size(rocks_table_props_collection_t* coll) { return coll->rep.size(); }
...
rocks_table_props_collection_iter_t* rocks_table_props_collection_iter_create(rocks_table_props_collection_t* coll) {
  if (!coll->rep.empty()) {
    return new rocks_table_props_collection_iter_t{coll->rep.cbegin(), coll->rep.cend()};
  }
  return nullptr;
}
...
uint64_t rocks_table_props_get_data_size(rocks_table_props_t* prop) { return prop->rep->data_size; }
uint64_t rocks_table_props_get_index_size(rocks_table_props_t* prop) { return prop->rep->index_size; }
uint64_t rocks_table_props_get_filter_size(rocks_table_props_t* prop) { return prop->rep->filter_size; }
uint64_t rocks_table_props_get_raw_key_size(rocks_table_props_t* prop) { return prop->rep->raw_key_size; }
uint64_t rocks_table_props_get_raw_value_size(rocks_table_props_t* prop) { return prop->rep->raw_value_size; }
uint64_t rocks_table_props_get_num_data_blocks(rocks_table_props_t* prop) { return prop->rep->num_data_blocks; }
uint64_t rocks_table_props_get_num_entries(rocks_table_props_t* prop) { return prop->rep->num_entries; }
uint64_t rocks_table_props_get_format_version(rocks_table_props_t* prop) { return prop->rep->format_version; }
uint64_t rocks_table_props_get_fixed_key_len(rocks_table_props_t* prop) { return prop->rep->fixed_key_len; }
uint32_t rocks_table_props_get_column_family_id(rocks_table_props_t* prop) { return prop->rep->column_family_id; }
...
rocks_user_collected_props_t* rocks_table_props_get_user_collected_properties(rocks_table_props_t* prop) {
  auto ptr = reinterpret_cast<const void*>(&prop->rep->user_collected_properties);
...
rocks_user_collected_props_iter_t* rocks_user_collected_props_iter_create(rocks_user_collected_props_t* prop) {
  auto user_prop = reinterpret_cast<UserCollectedProperties*>(prop);
  if (!user_prop->empty()) {
    return new rocks_user_collected_props_iter_t{user_prop->cbegin(), user_prop->cend()};
  }
  return nullptr;
}
